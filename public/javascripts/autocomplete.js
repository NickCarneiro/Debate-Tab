			...
			for (var i = 0; i < result.length; i++) {
				...
				li_em.mousedown(function() {
					...
					if (options.onselect) {
						...
						//on click event, fill input value with attr and
						//bind object id to input dom elemnt
						$this.data("id", model.get("id"));
			...
			if (e.keyCode == 13 || e.keyCode == 9) {
				...
				if (em.length) {
					...
					if (options.onselect) {
						...
						//set input value to attr and bind data value for associated id
						//do same thing on click event					
						this.data({"id": model.get("id")});
